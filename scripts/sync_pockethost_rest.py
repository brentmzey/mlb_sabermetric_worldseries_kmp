...
def _try_admin_endpoint(auth_ep: str) -> Optional[str]:
    """Tries authentication on a single admin endpoint."""
    try:
...
def authenticate_admin() -> str:
    ...
    for attempt in range(5):
        ...
        for auth_ep in ["/api/collections/_superusers/auth-with-password", "/api/admins/auth-with-password"]:
            token: Optional[str] = _try_admin_endpoint(auth_ep)
            if token:
...
def _import_collection_schemas(token: str) -> None:
    """Imports or updates collection schemas with panel data fields."""
...
def _fetch_existing_teams_map(token: str) -> Dict[str, str]:
    """Fetches already-registered team code -> record ID mapping from tbl_mlb_teams."""
...
def _sync_master_teams(token: str, rows: Sequence[PanelDatasetCsvRow]) -> Dict[str, str]:
    """Ensures all 30 MLB franchises exist in `tbl_mlb_teams` and returns the ID map."""
    print("⚾ Syncing 30 MLB Teams to `tbl_mlb_teams`...")
    teams_by_code: Dict[str, str] = _fetch_existing_teams_map(token)
...
def _record_historical_simulation_run(yr: int, current_year: int, token: str) -> Optional[str]:
    """Creates a historical simulation run entry in `tbl_simulation_runs`."""
    ...
    try:
        ...
        if isinstance(run_rec, dict) and "id" in run_rec:
            return str(run_rec["id"])
    except Exception as e:
        print(f"   Note: Run {run_id} setup: {e}")
    return None


def _sync_single_team_snapshot_and_movement(
    yr: int,
    ...
    """Uploads a snapshot and rank movement record for a single team in a given season."""
    ...
    time.sleep(0.05)
    return snap_ok, move_ok


def _sync_single_season(
    yr: int,
    current_year: int,
    rows: Sequence[PanelDatasetCsvRow],
    teams_by_code: Mapping[str, str],
    token: str
) -> Tuple[int, int, int]:
    """Synchronizes a single season's run, snapshots, and movement records."""
    run_record_id: Optional[str] = _record_historical_simulation_run(yr, current_year, token)
    ...
    for team_row in rows:
        ...
        snap_ok, move_ok = _sync_single_team_snapshot_and_movement(yr, team_row, run_record_id, team_rec_id, token)
        if snap_ok:
            snaps_added += 1
        if move_ok:
            moves_added += 1

    return runs_added, snaps_added, moves_added


def main() -> None:
    """Main execution function for historical panel data synchronization."""
    token: str = authenticate_admin()
    _import_collection_schemas(token)
    ...
    teams_by_code: Dict[str, str] = _sync_master_teams(token, rows)
    ...
    yr: int
    for yr in seasons:
        r_add, s_add, m_add = _sync_single_season(yr, current_year, rows, teams_by_code, token)
        total_runs += r_add
        total_snaps += s_add
        total_moves += m_add
